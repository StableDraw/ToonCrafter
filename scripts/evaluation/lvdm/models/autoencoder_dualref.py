...
class Decoder(nn.Module):
    ...
    def forward(self, z, ref_context=None, timesteps = None, **kwargs):
        ...
        # middle
        h = self.mid.block_1(h, temb, timesteps = timesteps, **kwargs)
        h = self.mid.attn_1(h, **kwargs)
        h = self.mid.block_2(h, temb, timesteps = timesteps, **kwargs)

        # upsampling
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = self.up[i_level].block[i_block](h, temb, timesteps = timesteps, **kwargs)
        ...
        h = self.conv_out(h, timesteps = timesteps, **kwargs)
        if self.tanh_out:
            h = torch.tanh(h)
        return h
