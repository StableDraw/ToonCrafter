...
def run_inference(args, gpu_num, gpu_no):
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(prompt_list_rank), args["bs"])), desc='Sample Batch'):
            ...
            batch_samples = image_guided_synthesis(model, prompts, videos, noise_shape, args["n_samples"], args["ddim_steps"], args["ddim_eta"], args["unconditional_guidance_scale"], args["cfg_img"], args["frame_stride"], args["text_input"], args["multiple_cond_cfg"], args["loop"], args["interp"], args["timestep_spacing"], args["guidance_rescale"])
...
if __name__ == '__main__':
    ...
    args = {
        "savedir": "results/tooncrafter_512_interp_seed123", #results saving path
        "ckpt_path": "model.ckpt", #checkpoint path
        "config": "configs/inference_512_v1.0.yaml",#config (yaml) path
        "prompt_dir": "prompts/512_interp/", #a data dir containing videos and prompts
        ...
        "text_input": False, #input text to I2V model or not
